...
class Auction(QObject):
    ...
    def setupbrowser(self):
        ...
        try:
            print("브라우저 초기화 시작...")
            self.auction_operation_signal.emit("브라우저 초기화 중...")
            
            # 방법 1: undetected-chromedriver (버전 지정)
            try:
                options = uc.ChromeOptions()
                
                # 기본 설정
                options.add_argument('--headless=new')  # 이 라인 추가
                ...
                # Chrome 138 버전에 맞는 ChromeDriver 사용
                self.browser = uc.Chrome(
                    options=options,
                    use_subprocess=False,
                    version_main=138,  # 현재 Chrome 버전에 맞춤
                    patcher_force_close=True
                )
                
                print("undetected-chromedriver로 브라우저 초기화 성공!")
                
            except Exception as e1:
                print(f"undetected-chromedriver 실패: {e1}")
                
                # 방법 2: 일반 webdriver with webdriver_manager
                try:
                    from selenium.webdriver.chrome.options import Options
                    from selenium.webdriver.chrome.service import Service
                    from webdriver_manager.chrome import ChromeDriverManager
                    ...
                    normal_options.add_argument('--window-size=1920,1080')
                    normal_options.add_argument(f'--user-data-dir={user_data_dir}')
                    normal_options.add_argument('--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36')
                    
                    # Chrome 138에 맞는 ChromeDriver 자동 다운로드
                    service = Service(ChromeDriverManager(chrome_type=None).install())
                    self.browser = webdriver.Chrome(service=service, options=normal_options)
                    
                    print("일반 webdriver로 브라우저 초기화 성공!")
                    
                except Exception as e2:
                    print(f"일반 webdriver도 실패: {e2}")
                    
                    # 방법 3: 기존 chromedriver.exe 사용
                    try:
                        normal_options = Options()
                        normal_options.add_argument('--no-sandbox')
                        normal_options.add_argument('--disable-dev-shm-usage')
                        ...
                        # 프로젝트 폴더에 있는 chromedriver.exe 사용
                        service = Service(executable_path='chromedriver.exe')
                        self.browser = webdriver.Chrome(service=service, options=normal_options)
                        
                        print("기존 chromedriver.exe로 브라우저 초기화 성공!")
            ...
            print("브라우저 초기화 성공!")
            self.auction_operation_signal.emit("브라우저 초기화 완료")
            return self.browser

        except Exception as e:
            import traceback
            error_detail = traceback.format_exc()
            error_msg = f"브라우저 설정 실패: {str(e)}\n상세 오류:\n{error_detail}"
            print(error_msg)
